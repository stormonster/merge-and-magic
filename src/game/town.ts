...
const TOWN_RARITY_WEIGHTS: Record<
  Extract<Rarity, "common" | "uncommon" | "rare">,
  number
> = {
  common: 58,
  uncommon: 32,
...
const TOWN_PRICE_RANGES: Record<
  Extract<Rarity, "common" | "uncommon" | "rare">,
  { min: number; max: number; levelMin: number; levelMax: number }
> = {
...
export function enterTown(state: GameState): EnterTownResult {
  ...
  if (isHealing(state)) {
    addLogEntry(
      state,
      "system",
      "🏘 The road to town is closed while recovering. A commit restores you immediately.",
    );
    return "healing";
  }
  ...
  addLogEntry(
    state,
    "system",
    `🏘 Entered town. Resting at the inn while shopping.`,
  );
  return "entered";
}
...
export function selectTownRarity(): Extract<
  Rarity,
...
export function getTownItemCost(
  playerLevel: number,
  rarity: Extract<Rarity, "common" | "uncommon" | "rare">,
): number {
  const range = TOWN_PRICE_RANGES[rarity];
  return randomInt(
    range.min + playerLevel * range.levelMin,
    range.max + playerLevel * range.levelMax,
  );
}
...
export function applyTownHealing(state: GameState): boolean {
  const nextHp = Math.min(
    state.player.maxHp,
    state.player.hp + TOWN_HEAL_PER_TICK,
  );
...
export function processTownPurchase(state: GameState): TownPurchaseResult {
  ...
  if (cost > state.player.gold) {
    leaveTown(
      state,
      `Left town. ${item.name} cost ${cost} gold, but you only had ${state.player.gold}.`,
    );
  ...
  addLogEntry(
    ...
    "loot_equipped",
    result.replacedItem
      ? `🛒 Bought ${item.name} for ${cost} gold.\nReplaced: ${result.replacedItem.name}.`
      : `🛒 Bought ${item.name} for ${cost} gold.`,
    [
      { text: item.name, rarity: item.rarity },
      ...(result.replacedItem
        ? [
            {
              text: result.replacedItem.name,
              rarity: result.replacedItem.rarity,
            },
          ]
        : []),
    ],
